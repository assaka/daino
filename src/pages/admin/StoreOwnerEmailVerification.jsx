...
import { CheckCircle, Mail, AlertCircle, LogOut } from 'lucide-react';
import { createAdminUrl } from '@/utils/urlUtils';
import { setRoleBasedAuthData, handleLogout } from '@/utils/auth';

export default function StoreOwnerEmailVerification() {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const response = await apiClient.post('auth/store-owner/verify-email', {
        email,
  ...
  const handleResend = async () => {
    ...
    try {
      const response = await apiClient.post('auth/store-owner/resend-verification', { email });
      ...
      if (response.success) {
        ...
        setError(response.error || response.message || 'Failed to resend code');
      }
    } catch (err) {
        ...
        <div className="flex-1 flex items-center justify-center px-4">
          <div className="max-w-md w-full bg-white rounded-lg shadow-lg p-8 text-center">
            ...
            <p className="text-gray-600 mb-4">
              Your email has been successfully verified. Redirecting to store setup...
            </p>
          </div>
        </div>
      </div>
