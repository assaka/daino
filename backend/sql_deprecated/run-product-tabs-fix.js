...
/**
 * Fix product_tabs timestamp defaults on production database
 *
 * Usage:
 * SUPABASE_DB_URL=your_db_url JWT_SECRET=your_secret node run-product-tabs-fix.js
 */
...
async function runProductTabsFix() {
  ...
  if (missing.length > 0) {
    ...
    console.log('\n💡 Usage:');
    console.log('SUPABASE_DB_URL=your_db_url JWT_SECRET=your_secret node run-product-tabs-fix.js');
    process.exit(1);
  }

  try {
    console.log('🚀 Starting product_tabs timestamp fix migration...');
    ...
    const migrationPath = path.join(__dirname, 'fix-product-tabs-timestamps.sql');
    const migrationSQL = fs.readFileSync(migrationPath, 'utf8');

    console.log('📄 Product tabs fix migration file loaded');
    ...
    console.log('\n🎉 Product tabs fix migration completed!');
    console.log(`📊 Results: ${successCount} executed, ${skipCount} skipped`);

    // Verify the migration by checking product_tabs table structure
    console.log('\n🧪 Testing product_tabs table structure...');
    try {
      ...
      console.log('📊 Product tabs timestamp columns:');
      results.forEach(col => {
      ...
      if (createdHasDefault && updatedHasDefault) {
        console.log('\n🎉 Product tabs timestamp fix successful!');
      } else {
    ...
    console.error('\n❌ Product tabs fix migration failed:', error.message);
