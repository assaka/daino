/**
 * WooCommerceSchedule Model - UNIFIED VERSION
 *
...
/**
 * Convert cron_jobs record to WooCommerceSchedule format for API compatibility
 */
...
const WooCommerceSchedule = {
  tableName: 'cron_jobs',
  sourceType: 'integration',
  sourceName: 'woocommerce',

  /**
   * Create a new WooCommerce schedule (stored in cron_jobs)
   */
  async create(scheduleData) {
    ...
    const cronJobData = {
      id: uuidv4(),
      name: `WooCommerce ${import_type} Import`,
      description: `Scheduled ${import_type} import from WooCommerce`,
      cron_expression: cronExpression,
      timezone: 'UTC',
      job_type: 'woocommerce_import',
      configuration: {
        import_type,
        options
      },
      ...
      handler: 'executeWooCommerceImport',
      store_id,
      is_active,
      is_paused: false,
      is_system: false,
    ...
    if (error) {
      console.error('Error creating WooCommerce schedule:', error);
      throw new Error(`Failed to create schedule: ${error.message}`);
    }

    console.log(`Created WooCommerce schedule: ${data.id} (${import_type})`);
  ...
  async destroy(id, storeId) {
    ...
    const { error } = await tenantDb
      ...
      .delete()
      .eq('id', id)
      .eq('job_type', 'woocommerce_import');
    ...
    console.log(`Deleted WooCommerce schedule: ${id}`);
    return true;
  },
  ...
  async findAll(options = {}) {
    ...
    // Query for WooCommerce schedules - match by job_type
    let query = tenantDb
      .from(this.tableName)
      .select('*')
      .eq('store_id', store_id)
    ...
    console.log(`[WooCommerceSchedule.findAll] Querying for store_id: ${store_id}`);
    ...
    console.log(`[WooCommerceSchedule.findAll] Found ${data?.length || 0} schedules`);
  ...
  async update(id, updateData, storeId) {
    ...
    console.log(`Updated WooCommerce schedule: ${id}`);
    return toCronJobToScheduleFormat(data);
  },
   ...
   * Find schedule by id
   */
  async findByPk(id, storeId) {
    if (!storeId) {
      throw new Error('storeId is required for findByPk');
    }
...
module.exports = WooCommerceSchedule;
