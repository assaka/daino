...
class SupabaseIntegration {
  ...
  /**
   * Process pending OAuth tokens in the background (non-blocking)
   * This is called after status check returns to avoid blocking
   */
  async _processPendingTokensInBackground(storeId, tokenData, source) {
    try {
      console.log(`🔄 [BACKGROUND] Processing pending OAuth tokens from ${source}...`);
      ...
      try {
        ...
        console.log('✅ [BACKGROUND] Token synced to integration_tokens');
      } catch (syncError) {
        console.warn('[BACKGROUND] Failed to sync token expiry:', syncError.message);
      }

      // Insert into store_databases in master DB
      if (tokenData.project_url && tokenData.service_role_key) {
        try {
          ...
          const { error: storeDbError } = await masterDbClient
            .from('store_databases')
            .upsert({
              id: uuidv4(),
              store_id: storeId,
          ...
          if (storeDbError) {
            console.error('❌ [BACKGROUND] store_databases insert failed:', storeDbError.message);
          } else {
            console.log('✅ [BACKGROUND] Inserted into store_databases');
          }
        } catch (storeDbInsertError) {
          console.error('❌ [BACKGROUND] store_databases error:', storeDbInsertError.message);
        }
      }
      ...
      if (source === 'redis') {
        ...
        if (redisClient) {
          ...
          console.log('🧹 [BACKGROUND] Cleaned up Redis');
        }
      } else if (source === 'memory' && global.pendingOAuthTokens) {
        global.pendingOAuthTokens.delete(storeId);
        console.log('🧹 [BACKGROUND] Cleaned up memory');
      }

      console.log('✅ [BACKGROUND] Pending token processing complete');
    } catch (error) {
      console.error('❌ [BACKGROUND] Error processing pending tokens:', error.message);
    }
  }

  /**
   * Get connection status
   * Optimized for speed - returns quickly and processes pending tokens in background
   */
  async getConnectionStatus(storeId) {
    try {
      ...
      let pendingTokenSource = null;

      try {
        config = await IntegrationConfig.findByStoreAndType(storeId, this.integrationType);

        // Check Redis for pending OAuth tokens (quick check only)
        try {
          const { getRedisClient } = require('../config/redis');
          const redisClient = getRedisClient();
          ...
          if (redisClient) {
            ...
            if (tokenDataStr) {
              console.log('🔄 Found pending OAuth tokens in Redis');
              pendingTokenData = JSON.parse(tokenDataStr);
              pendingTokenSource = 'redis';

              // Use pending token data immediately for status response
              token = {
                access_token: pendingTokenData.access_token,
                refresh_token: pendingTokenData.refresh_token,
                service_role_key: pendingTokenData.service_role_key,
                project_url: pendingTokenData.project_url,
                expires_at: pendingTokenData.expires_at
              };
            }
          }
        } catch (redisError) {
          console.log('[getConnectionStatus] Redis check failed:', redisError.message);
        }

        // Check memory fallback for pending tokens
        if (!pendingTokenData && global.pendingOAuthTokens && global.pendingOAuthTokens.has(storeId)) {
          console.log('🔄 Found pending OAuth tokens in memory');
          pendingTokenData = global.pendingOAuthTokens.get(storeId);
          pendingTokenSource = 'memory';
        ...
        // If no pending tokens were found, extract token from config we already fetched
        // This avoids a duplicate query since getSupabaseToken would query the same table
        if (!token && config && config.config_data) {
          token = {
            id: config.id,
            store_id: storeId,
            access_token: config.config_data.accessToken,
      ...
      // Process pending tokens in background (non-blocking) if found
      if (pendingTokenData && pendingTokenSource) {
        // Fire and forget - don't await
        this._processPendingTokensInBackground(storeId, pendingTokenData, pendingTokenSource)
          .catch(err => console.error('[getConnectionStatus] Background processing error:', err.message));
      }
      ...
      // Check if authorization was revoked - handle in background too
      if (config && config.connection_status === 'failed' && config.config_data?.revokedDetected) {
        ...
        (async () => {
          try {
            ...
            console.error('[getConnectionStatus] Background revocation cleanup error:', err.message);
          }
        })();
      ...
      // Skip connection verification - trust stored credentials
      // Actual connection issues will be caught on usage, not during status check
      let connectionVerified = true; // Assume verified if we have token
